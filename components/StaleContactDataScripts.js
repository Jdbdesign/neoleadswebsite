...
// Page-specific behaviour for /stale-contact-data.
...
// contract on each .scd-metric-num:
//   data-to="22.5"        target value
//   data-decimals="1"     decimal places to render
//   data-suffix="%"       text appended after the number (e.g. "%", " hrs")
//   data-prefix="$"       text prepended before the number (optional)

import { useEffect } from 'react';

export default function StaleContactDataScripts() {
  useEffect(() => {
    const grid = document.querySelector('.scd-metrics-grid');
    if (!grid) return;

    const nums = Array.prototype.slice.call(grid.querySelectorAll('.scd-metric-num'));
