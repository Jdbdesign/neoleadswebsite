...
// "Sendrit + NeoBrain AI — Only clean contacts send" step card (card 4 of the
// /stale-contact-data "Full Fix" bento). Reuses the multichannel cadence
// interaction from the /low-reply-rates "Send & follow up" card, adapted to this
// card's .scd-fix-* frame. The OUTER card (badge, heading, body) runs a one-time
// scroll entrance and then never moves. The INNER sequence panel loops
// continuously, replaying a story-driven cadence:
//   Day 1 — Email fires (send-line down the wire, "sent")
//   …time passes (· · ·)…
//   Day 3 — LinkedIn opened (notification ripple ping, "opened")
//   …time passes (· ·)…
//   Day 6 — Reply arrives (purple flood, icon spring, "Interested" types in
//           char-by-char, arrival glow) — the money moment.
...
// via the unique `scdsnd` prefix so nothing leaks into global styles.
//
...
const WHITE = '#FFFFFF';
const ORANGE = '#F59E0B';
const PURPLE = 'var(--brand-light)';
const BORDER = 'var(--bg-card-border)';
const PURPLE_BORDER = 'rgba(139,77,255,0.45)';
const ACTIVE_BG = 'rgba(108,43,223,0.2)';
...
export default function StaleContactDataSendCard() {
  ...
  useEffect(() => {
    ...
    const showTimeDots = async (container, cycles) => {
      ...
      spans.forEach((s, i) => {
        s.style.animation = `scdsndDot 600ms ease-in-out ${i * 150}ms ${cycles}`;
      });
    ...
    const fireLinkedInRow = async () => {
      ...
      if (ic) {
        ...
        void ic.offsetWidth;
        ic.style.animation = 'scdsndRipple 500ms ease-out forwards';
        later(() => {
    ...
    const fireReplyRow = async () => {
      ...
      pulseScale(icon3Ref.current, 1.15); // reply earns its moment
      ...
      if (cursorRef.current) {
        ...
        cursorRef.current.style.animation = 'scdsndBlink 1060ms step-end infinite';
      }
    ...
    const hold = async (ms) => {
      ...
      animate(2500, (t) => {
        const a = 0.2 + 0.1 * Math.sin(Math.PI * t);
        if (row3Ref.current) row3Ref.current.style.background = `rgba(108,43,223,${a})`;
      }).then(() => {
        if (row3Ref.current) row3Ref.current.style.background = ACTIVE_BG;
      });
      await pause(ms);
      setWill(true);
    };
    ...
    const renderFinal = () => {
      ...
      if (row3Ref.current) {
        ...
        row3Ref.current.style.background = ACTIVE_BG;
  ...
  100% { opacity: 0.2; }
      ...
      <div className="scd-fix-demo" ref={mockRef} style={{ opacity: 0, transform: 'translateY(10px)' }}>
        <div className="scd-fix-seq">
          {/* Row 1 — Email */}
          <div ref={row1Ref} className="scd-fix-seq-row" style={{ ...rowBase, overflow: 'hidden' }}>
            <span ref={icon1Ref} className="scd-fix-seq-ic" style={{ transition: ICON_TRANSITION }}>
              <Icon name="mail" aria-hidden="true" />
            </span>
            <span ref={lbl1Ref} className="scd-fix-seq-label">Email &middot; Day 1</span>
            <span ref={st1Ref} className="scd-fix-seq-stat gray" style={{ opacity: 0 }}>Sent</span>
          </div>
          ...
          {/* Row 2 — LinkedIn */}
          <div ref={row2Ref} className="scd-fix-seq-row" style={rowBase}>
            <span ref={icon2Ref} className="scd-fix-seq-ic" style={{ transition: ICON_TRANSITION }}>
              <Icon name="linkedin" aria-hidden="true" />
            </span>
            <span ref={lbl2Ref} className="scd-fix-seq-label">LinkedIn &middot; Day 3</span>
            <span ref={st2Ref} className="scd-fix-seq-stat amber" style={{ opacity: 0 }}>Opened</span>
          </div>
          ...
          {/* Row 3 — Reply */}
          <div ref={row3Ref} className="scd-fix-seq-row" style={rowBase}>
            <span ref={icon3Ref} className="scd-fix-seq-ic" style={{ transition: ICON_TRANSITION }}>
              <Icon name="message-circle" aria-hidden="true" />
            </span>
            <span ref={lbl3Ref} className="scd-fix-seq-label">Reply &middot; Day 6</span>
            <span className="scd-fix-seq-stat purple" style={{ display: 'inline-flex', alignItems: 'center' }}>
