...
// Page-specific behaviour for /manual-prospecting.
...
// contract on each .mp-metric-num:
//   data-to="70"          target value
//   data-decimals="0"     decimal places to render
//   data-suffix="%"       text appended after the number (e.g. "%", " weeks", "×")
//   data-prefix=""        text prepended before the number (optional)

import { useEffect } from 'react';

export default function ManualProspectingScripts() {
  useEffect(() => {
    const grid = document.querySelector('.mp-metrics-grid');
    if (!grid) return;

    const nums = Array.prototype.slice.call(grid.querySelectorAll('.mp-metric-num'));
