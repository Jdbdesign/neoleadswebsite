...
// Page-specific behaviour for /missed-hot-replies.
...
// contract on each .mhr-metric-num:
//   data-to="42"          target value
//   data-decimals="0"     decimal places to render
//   data-suffix=" hrs"    text appended after the number (e.g. "%", " hrs", "×")
//   data-prefix=""        text prepended before the number (optional)

import { useEffect } from 'react';

export default function MissedHotRepliesScripts() {
  useEffect(() => {
    const grid = document.querySelector('.mhr-metrics-grid');
    if (!grid) return;

    const nums = Array.prototype.slice.call(grid.querySelectorAll('.mhr-metric-num'));
