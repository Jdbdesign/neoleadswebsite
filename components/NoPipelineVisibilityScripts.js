...
// Page-specific behaviour for /no-pipeline-visibility.
...
// contract on each .npv-metric-num:
//   data-to="54"          target value
...
export default function NoPipelineVisibilityScripts() {
  useEffect(() => {
    const grid = document.querySelector('.npv-metrics-grid');
    if (!grid) return;

    const nums = Array.prototype.slice.call(grid.querySelectorAll('.npv-metric-num'));
