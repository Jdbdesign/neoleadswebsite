...
// Page-specific behaviour for /unqualified-lead-lists.
...
// contract on each .ull-metric-num:
//   data-to="27"          target value
...
export default function UnqualifiedLeadListsScripts() {
  useEffect(() => {
    const grid = document.querySelector('.ull-metrics-grid');
    if (!grid) return;

    const nums = Array.prototype.slice.call(grid.querySelectorAll('.ull-metric-num'));
