...
// Page-specific behaviour for /high-bounce-rates.
// Drives count-up animations on two bands the first time each scrolls into view:
//   1. the hero comparison stats (.hbr-compare-grid .hbr-stat-num)
//   2. the data band metrics      (.hbr-metrics-grid .hbr-metric-num)
// Markup contract on each animated number:
//   data-to="84"          target value
//   data-decimals="1"     decimal places to render (optional, default 0)
//   data-suffix="%"       text appended after the number (optional)
//   data-prefix=""        text prepended before the number (optional)
// Integer parts are grouped with thousands separators (1000 -> "1,000").

import { useEffect } from 'react';

export default function HighBounceRatesScripts() {
  useEffect(() => {
    ...
    const observers = [
      wireBand('.hbr-compare-grid', '.hbr-stat-num[data-to]'),
      wireBand('.hbr-metrics-grid', '.hbr-metric-num[data-to]'),
    ].filter(Boolean);
